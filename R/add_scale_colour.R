#' @title add a scale_colour palette to a ggplot
#' @description helper function to `plot_rast_gg` and `plot_vect` which adds
#'  and customizes the scale_colour palette based on characteristics of the plot
...
#' @param ... any other parameter to be passed to `ggplot::scale_colour_brewer` or
#'  `ggplot::scale_colour_hue`
#' @return The function is called for its side effects.
#' @rdname add_scale_colour
#' @author Lorenzo Busetto, phD (2017) <lbusett@gmail.com>
#' @importFrom scales squish censor
#' @importFrom ggplot2 aes_string scale_colour_brewer scale_colour_hue
#'   scale_colour_distiller scale_colour_manual


add_scale_colour <- function(plot,
                           palette,
                           title,
                           na.color,
                           zlims,
  ...
  # Qualitative palette: use scal_fill_hue or scale_colour_brewer             ####
  ...
  if (palette[["cont_qual"]] == "qual") {
    ...
    # TODO add support for scale_colour_viridis and maybe scale_colour_gradient

    if (palette$source == "brewer") {
      plot <- plot + scale_colour_distiller(
        title,
        limits  = zlims,
