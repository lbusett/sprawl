...
#' @param zone_object `sf` object on which features the zonal statistics has to be computed
...
cz_points <- function(zone_object,
                     in_rast,
                     n_selbands,
                     selbands,
                     seldates,
  ...
  tserie <- matrix(nrow = n_selbands, ncol = dim(zone_object_crop)[1])
  ...
  for (band in seq_len(n_selbands)) {
    ...
    tserie[band,] <- in_rast[[selband]] %>%
      raster::extract(as(zone_object_crop, "Spatial"))
  }
  ...
  all_feats <- as.character(zone_object_crop$mdxtnq)
  ...
  if (long) {
    ...
    tserie <- tserie %>%
      ...
      dplyr::right_join(tibble::as_data_frame(zone_object), by = "mdxtnq") %>%
      sf::st_as_sf()
    ...
    if (!is.null(id_field) ){
      if (keep_null) {
        names(tserie)[3:(dim(tserie)[2])] <- t(rbind(tibble::as_data_frame(zone_object[, eval(id_field)])[,1]))
      } else {
        names(tserie)[3:dim(tserie)[2]] <- t(rbind(tibble::as_data_frame(zone_object_crop[, eval(id_field)])[,1]))
      }
    }
