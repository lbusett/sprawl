...
er_crop_object <- function(in_vect,
  ...
  #                          sf::st_crs(in_vect))
  #
  # in_vect_crop <- sf::st_intersection(in_vect, st_geometry(ext_sf))
  #
  # in_vect_crop <- sf::st_intersection(st_geometry(in_vect),
  #                                           st_geometry(ext_sf))
  ...
  Outputs for features outside rasterstack extent will be set to NA."
    ...
    if (!setequal(in_vect$mdxtnq, in_vect_crop$mdxtnq)) {
      ...
      outside_names <- ifelse(!is.null(id_field),
                              as.character(tibble::as_data_frame(in_vect[outside_ids,eval(id_field)])[,1]),
