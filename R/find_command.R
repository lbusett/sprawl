...
#' @description helper function allowing to find the path to a given command on the system of the
#' user. If the command is not found, a `stop` is issued and the program aborts (gracefully).
...
find_command <- function(command,
  ...
  if ((command_path == "")) {
    stop("find_command --> `", command, "` was not found on your system. Please install it and/or check
           your PATH environment variables. Aborting !")
  } else {
