...
[![codecov](https://codecov.io/gh/IREA-CNR-MI/sprawl/branch/devel/graph/badge.svg)](https://codecov.io/gh/IREA-CNR-MI/sprawl)
...
4. Optimizing code execution speed by providing under-the-hood support
for multi-core processing using for exmaple `foreach` looping and `raster::MakeCluster` functionality
...
We are open for contributions/help in improving `sprawl` and extend its functionalities. If you have a suggestion, you can report it in our [issues] page, or fork the repository and then make a pull request.
(note that we plan to provide som "good-practices" instructions for contributing new functions in the future.)

## Citation

To cite package `sprawl` in publications use:

  Lorenzo Busetto and Luigi Ranghetti (2017). sprawl: Spatial Processing (in) R: Amhorphous
