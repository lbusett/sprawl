...
testthat::test_that("Test On polygons extraction", {
  ...
  out <- extract_rast(in_rast, in_polys, selbands = c("2013-01-01","2013-01-08"), verbose = FALSE)
  expect_is(out, "list")
  ...
  out  <- extract_rast(in_rast, in_polys, verbose = F, keep_null = T, selbands = c(1,2), small = F)
  out2 <- extract_rast(in_rast, in_polys, verbose = F, keep_null = T, selbands = c(1,2), maxchunk = 30000,  small = F)
  ...
  out    <- extract_rast(in_rast, in_polys, verbose = F, keep_null = T, selbands = c(1,2), small = T, id_field = "id")
  out2   <- extract_rast(in_rast, in_polys, verbose = F, keep_null = T, selbands = c(1,2), small = T, id_field = "lc_type",
                         addfeat = FALSE)
  # out2  <- extract_rast(in_rast, in_polys, verbose = F, long = T, keep_null = T, selbands = c(1,2), small = T, id_field = "lc_type")
  out3  <- expect_warning(extract_rast(in_rast, in_polys, verbose = F, keep_null = T, selbands = c(1,2), small = T, id_field = "lc_tydfse"))
  out4  <- extract_rast(in_rast, in_polys, verbose = F, keep_null = T, selbands = c(1,2), small = T)
  ...
  out2  <- extract_rast(in_rast, in_polys, verbose = F, keep_null = T, selbands = c(1,2), small = T, comp_quant = TRUE)
  expect_equal(out$stats$avg, out2$stats$avg)
  expect_equal(out$stats$sd, out2$stats$sd)

  # Check that results are coherent with `raster::extract` on the test dataset
  out_extract_rast  <- extract_rast(in_rast, in_polys, selbands = c(1,2), verbose = F, keep_null = T, addgeom = T, full_data = F, small = T,  comp_quant = FALSE)
  ...
  outcustom <- extract_rast(in_rast, in_polys, selbands = c(1,2), verbose = F, keep_null = T,
