...
testthat::test_that("Test On points extraction", {
  ...
  out      <- extract_rast(in_rast, in_pts, id_field = "id", verbose = FALSE, keep_null = T)  %>%
                   tibble::as_tibble()
  ...
  out <- extract_rast(in_rast, in_pts, id_field = "id", verbose = F, keep_null = T, long = T)
  # On `long = TRUE` output is `sf`
  testthat::expect_is(out, "sf")

  # On `long = TRUE` but add_geom = FALSE output is not a `sf`
  out <- extract_rast(in_rast, in_pts, id_field = "id", verbose = F, addgeom = F)
