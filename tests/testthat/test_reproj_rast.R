test_that("reproj_rast works as expected", {
  ...
  out1 <- expect_is(reproj_rast(in_file, out_proj, out_file =
                                  tempfile(fileext = ".tif")), "Raster")
  out2 <- reproj_rast(read_rast(in_file), out_proj, out_file =
  ...
  out3 <- expect_is(reproj_rast(in_file, "+init=epsg:4326", out_file =
                                  tempfile(fileext = ".tif")), "Raster")
