...
testthat::test_that("Test On points extraction", {
  ...
  testthat::expect_warning(out <- comp_zonal(in_rast, in_pts, id_field = "id", verbose = F, long = T, keep_null = T) )
  # On `long = TRUE` output is `sf`
  testthat::expect_is(out, "sf")

  # On `long = TRUE` but add_geom = FALSE output is not a `sf`
  testthat::expect_warning(out <- comp_zonal(in_rast, in_pts, id_field = "id", verbose = F, long = T, addgeom = F) )
