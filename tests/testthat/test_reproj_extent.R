...
testthat::test_that("Test reproj_extent",{
  ...
  ext_reproj_3 <- reproj_extent(raster::extent(in_rast),
                                "+init=epsg:32651", in_proj=in_rast@crs)
  testthat::expect_is(ext_reproj_3, "sprawlext")
  testthat::expect_equal(ext_reproj_3, ext_reproj_1)

  # All ok - reproj without enlarging
  ext_reproj_4 <- reproj_extent(in_sprawlext_1, "+init=epsg:32651",
                                enlarge = FALSE)
  expect_equal(sf::st_covers(as(ext_reproj_1,"sfc_POLYGON"),
                             as(ext_reproj_4,"sfc_POLYGON"),
                             sparse = FALSE),
               matrix(TRUE))
  ...
  testthat::expect_error(
    reproj_extent(extent(in_rast), "+init=epsg:432ds6", in_proj = "+init=epsg:3sss035"))  #nolint
