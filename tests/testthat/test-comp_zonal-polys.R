...
testthat::test_that("Test On polygons extraction", {
  ...
  expect_warning(out <- comp_zonal(in_rast, in_polys, selbands = c("2013-01-01","2013-01-08"), verbose = FALSE))
  expect_is(out, "list")
  expect_warning(out <- comp_zonal(in_rast, in_polys, selbands = c(1,2), verbose = FALSE))
  expect_is(out, "list")
